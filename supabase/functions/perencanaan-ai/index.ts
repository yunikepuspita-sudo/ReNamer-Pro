// Edge Function: perencanaan-ai
// Relay AI untuk "AI Planning Document Factory — KPU".
// Menerima prompt yang sudah dirangkai klien (system + messages) lalu memanggil
// Claude (Anthropic). Menjaga ANTHROPIC_API_KEY tetap di server (tidak di browser).
//
...
//
// Deploy: supabase functions deploy perencanaan-ai --no-verify-jwt
...
// Daftar model yang diizinkan (mencegah penyalahgunaan parameter model).
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const max_tokens: number = Math.min(Math.max(Number(body.max_tokens) || 2000, 256), 8000)
