...
export default function Reader() {
  ...
  const [chapterIndex, setChapterIndex] = useState<number>(book ? progress[book.id] ?? 0 : 0)
  const [tocOpen, setTocOpen] = useState(false)
  const [settingsOpen, setSettingsOpen] = useState(false)
  const [bookmarks, setBookmarks] = useState<number[]>([])

  const owned = book ? inLibrary(book.id) : false
  // Pengguna yang belum memiliki & non-premium hanya bisa membaca bab pertama (cuplikan).
  const previewOnly = book ? book.premium && !premium && !owned : false
  ...
  useEffect(() => {
    if (book) setProgress(book.id, chapterIndex)
